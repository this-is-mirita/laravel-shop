...
@section('content')
    <x-forms.auth-forms title="Регистрация" action="{{ route('registeruser') }}" method="post">
        @csrf
        {{--name--}}
        <x-forms.text-input
            name="name"
        ...
        <x-slot:socialAuth>
            <ul class="space-y-3 mt-3">
                <li>
                    <a href="{{ route('social.github') }}" class="relative flex items-center h-14 px-12 rounded-lg border border-[#A07BF0] bg-white/20 hover:bg-white/20 active:bg-white/10 active:translate-y-0.5">
